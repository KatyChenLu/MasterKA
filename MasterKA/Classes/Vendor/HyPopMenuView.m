...
- (void)initUIsize
{
    ...
    UIImageView* bottomView = [_backgroundView viewWithTag:2];
    
    if (!bottomView) {
        bottomView = [UIImageView new];
        if (iOS9Later&&iOS11Before) {
...
//    bottomView.frame = CGRectMake(0,ScreenHeight - 49 -8, ScreenWidth, 49);

//    bottomView.image = [UIImage imageNamed:@"tabbar_bg"];
    
    
    
    if (IsPhoneX) {
        bottomView.frame = CGRectMake(0, ScreenHeight - 88 -8, ScreenWidth, 88 + 8);
        bottomView.image = [[UIImage imageNamed:@""] resizableImageWithCapInsets:UIEdgeInsetsMake(20, 20, 20, 20) resizingMode:UIImageResizingModeStretch];
    }else{
        bottomView.frame =CGRectMake(0, ScreenHeight - 49 -8, ScreenWidth, 49);
...
//        [bottomView setBackgroundColor:[UIColor colorWithRed:90.f / 225.f green:90.f / 225.f blue:90.f / 225.f alpha:0.9f]];
    ...
    [disappearButton setImage:[UIImage imageNamed:CancelStrImgaeName] forState:UIControlStateNormal];
    [disappearButton addTarget:self  action:@selector(closeMenu) forControlEvents:UIControlEventTouchUpInside];
    disappearButton.bounds = CGRectMake(0, 0, 59, 59);
    if (IsPhoneX) {
        disappearButton.centerX = ScreenWidth/2;
        disappearButton.centerY = bottomView.centerY - 16;
        disappearButton.imageView.contentMode = UIViewContentModeCenter;
    }else{
        disappearButton.imageView.contentMode = UIViewContentModeCenter;
...
- (void)backgroundAnimate
{
    ...
    [UIView animateWithDuration:Duration animations:^{
        ...
        weakView.disappearButton.transform = CGAffineTransformMakeRotation((M_PI *3) / 4);
    }];
    ...
    [_dataSource enumerateObjectsUsingBlock:^(id _Nonnull obj, NSUInteger idx, BOOL* _Nonnull stop) {
        ...
        if (iOS9Later&&iOS11Before) {
            ...
            [weakView.superView addSubview:model.customView];
             ...
             [weakView addSubview:model.customView];
        }
...
{
    NSInteger column = self.columnNum;
    ...
    CGFloat buttonViewY = ((rowIndex + 1) * margin + rowIndex * buttonViewHeight) + (kH - toHeight) - 300;
