...
- (void)setStyleChangeButtonShow:(BOOL)styleChangeButtonShow
{
    _styleChangeButtonShow = styleChangeButtonShow;
    self.styleChangeButton.hidden = styleChangeButtonShow;
    [self updateSubviews];
}

- (void)setFaceButtonShow:(BOOL)faceButtonShow
{
    _faceButtonShow = faceButtonShow;
    self.faceButton.hidden = faceButtonShow;
    [self updateSubviews];
}

- (void)setImageButtonShow:(BOOL)imageButtonShow{
    _imageButtonShow = imageButtonShow;
    self.moreButton.hidden = imageButtonShow;
    [self updateSubviews];
}
- (void)setSendButtonShow:(BOOL)sendButtonShow {
    _sendButtonShow = sendButtonShow;
...
//        return ![textView deleteEmojiValue];
        return YES;
    }
}
...
{
//    NSString*  charText = self.inputTextView.text;
    if (isDelete) {
        [self.inputTextView deleteEmojiValue];
//        [self.inputTextView performSelector:@selector(deleteBackward) withObject:nil];
    }else{
        [self.inputTextView addEmojiValue:str];
//        self.inputTextView.text = [NSString stringWithFormat:@"%@%@",charText,str];
    }
...
- (void)setupSubviews
{
    ...
    self.styleChangeButton.hidden = self.styleChangeButtonShow;
    if (!self.styleChangeButton.hidden) {
    ...
    self.moreButton.hidden = self.imageButtonShow;
    ...
    self.faceButton.hidden = self.faceButtonShow;
    if (!self.faceButton.hidden) {
         allButtonWidth += CGRectGetWidth(self.faceButton.frame) + kHorizontalPadding * 1.5;
    }
   
    //发送
    self.sendButton = [[UIButton alloc] initWithFrame:CGRectMake(CGRectGetMinX(self.moreButton.frame) - ksendBtnMinWidth - kHorizontalPadding, kVerticalPadding, ksendBtnMinWidth, ksendBtnMinHeight)];
    self.sendButton.centerY = self.toolbarView.centerY;
    ...
    self.sendButton.backgroundColor = RGBFromHexadecimal(0xfeda0f);
    ...
    self.sendButton.hidden = self.sendButtonShow;
    ...
    if (!self.sendButton.hidden) {
        width = CGRectGetWidth(self.bounds) - ksendBtnMinHeight;
    }
    // 初始化输入框
...
//    self.inputTextView.contentMode = UIViewContentModeCenter;
    ...
    if (!self.faceView) {
        self.faceView = [[DXFaceView alloc] initWithFrame:CGRectMake(0, (kVerticalPadding * 2 + kInputTextViewMinHeight), self.frame.size.width, 216)];
        [(DXFaceView *)self.faceView setDelegate:self];
        self.faceView.backgroundColor = [UIColor colorWithRed:237/255.0f green:235/255.0f blue:240/255.0f alpha:1];
...
- (void)updateSubviews{
    if (self.styleChangeButton == nil) {
        return;
    }
    CGFloat allButtonWidth = 0.0;
    CGFloat textViewLeftMargin = 6.0;
    
    self.styleChangeButton.frame = CGRectMake(kHorizontalPadding, kVerticalPadding, kInputTextViewMinHeight, kInputTextViewMinHeight);
    if (!self.styleChangeButton.hidden) {
        allButtonWidth += CGRectGetMaxX(self.styleChangeButton.frame);
        textViewLeftMargin += CGRectGetMaxX(self.styleChangeButton.frame);
    }
    
    self.moreButton.frame = CGRectMake(CGRectGetWidth(self.bounds) - kHorizontalPadding - kInputTextViewMinHeight, kVerticalPadding, kInputTextViewMinHeight, kInputTextViewMinHeight);
    
    if (self.moreButton.hidden) {
        self.faceButton.frame = CGRectMake(CGRectGetWidth(self.bounds) - kHorizontalPadding - kInputTextViewMinHeight, kVerticalPadding, kInputTextViewMinHeight, kInputTextViewMinHeight);
        self.sendButton.frame = CGRectMake(CGRectGetWidth(self.bounds) - kHorizontalPadding - ksendBtnMinWidth, kVerticalPadding, ksendBtnMinWidth, ksendBtnMinHeight);
         self.sendButton.centerY = self.toolbarView.centerY;
    }else{
        self.faceButton.frame = CGRectMake(CGRectGetMinX(self.moreButton.frame) - kInputTextViewMinHeight - kHorizontalPadding, kVerticalPadding, kInputTextViewMinHeight, kInputTextViewMinHeight);
        self.sendButton.frame = CGRectMake(CGRectGetMinX(self.moreButton.frame) - ksendBtnMinWidth - kHorizontalPadding, kVerticalPadding, ksendBtnMinWidth, ksendBtnMinHeight);
         self.sendButton.centerY = self.toolbarView.centerY;
    }
    if (!self.faceButton.hidden) {
        allButtonWidth += CGRectGetWidth(self.faceButton.frame) + kHorizontalPadding * 1.5;
    ...
    if (!self.moreButton.hidden) {
        allButtonWidth += CGRectGetWidth(self.moreButton.frame) + kHorizontalPadding * 1.5;
    }else{
        allButtonWidth+=kHorizontalPadding;
    }
    ...
    CGFloat width = CGRectGetWidth(self.bounds) - (allButtonWidth ? allButtonWidth : (textViewLeftMargin * 2));
    // 初始化输入框
    self.inputTextView.frame = CGRectMake(textViewLeftMargin, kVerticalPadding, width, kInputTextViewMinHeight);
    self.recordButton.frame = CGRectMake(textViewLeftMargin, kVerticalPadding, width, kInputTextViewMinHeight);
    _previousTextViewContentHeight = [self getTextViewContentH:_inputTextView];
}
...
- (void)willShowInputTextViewToHeight:(CGFloat)toHeight
{
    ...
    NSLog(@"=====willShowInputTextViewToHeight  toHeight %f _previousTextViewContentHeight %f   %@",toHeight,_previousTextViewContentHeight,self.inputTextView);
...
//    && toHeight==self.inputTextView.frame.size.height
